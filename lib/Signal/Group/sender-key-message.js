...
class SenderKeyMessage extends ciphertext_message_1.CiphertextMessage {
    ...
    verifySignature(signatureKey) {
        ...
        const res = (0, curve_1.verifySignature)(Buffer.from(signatureKey), Buffer.from(part1), Buffer.from(part2));
        if (!res)
            throw new Error('Invalid signature!');
    }
    getSignature(signatureKey, serialized) {
        return Buffer.from((0, curve_1.calculateSignature)(Buffer.from(signatureKey), Buffer.from(serialized)));
    }
    serialize() {
