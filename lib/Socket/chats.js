...
const makeChatsSocket = (config) => {
    ...
    const profilePictureUrl = async (jid, type = 'preview', timeoutMs) => {
        ...
        const content = [{ tag: 'picture', attrs: { type, query: 'url' } }];
        await (0, tc_token_utils_1.buildTcTokenFromJid)({ keys: authState.keys, jid, baseContent: content });
        const result = await query({
            tag: 'iq',
            attrs: {
                target: jid,
    ...
    const presenceSubscribe = async (toJid, tcToken) => {
        ...
        if (tcToken) {
            ...
            content = await (0, tc_token_utils_1.buildTcTokenFromJid)({ keys: authState.keys, jid: toJid });
        }
        return sendNode({
            tag: 'presence',
            attrs: {
