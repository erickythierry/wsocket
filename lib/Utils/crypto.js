...
const generateSignalPubKey = (pubKey) => pubKey.length === 33 ? Buffer.from(pubKey) : Buffer.concat([Defaults_1.KEY_BUNDLE_TYPE, pubKey]);
...
exports.Curve = {
    ...
    sharedKey: (privateKey, publicKey) => {
        const shared = libsignal.curve.calculateAgreement((0, exports.generateSignalPubKey)(publicKey), Buffer.from(privateKey));
        return Buffer.from(shared);
    },
    sign: (privateKey, buf) => libsignal.curve.calculateSignature(Buffer.from(privateKey), Buffer.from(buf)),
    verify: (pubKey, message, signature) => {
        try {
            libsignal.curve.verifySignature((0, exports.generateSignalPubKey)(pubKey), Buffer.from(message), Buffer.from(signature));
            return true;
        }
        catch (error) {
            return false;
